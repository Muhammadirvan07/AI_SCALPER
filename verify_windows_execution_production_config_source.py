#!/usr/bin/env python3
"""Verify one Windows Execution production-config source with seven pins."""
...
def _parser() -> argparse.ArgumentParser:
    parser = _StableArgumentParser(
        description=(
            "Verify one deny-only Windows Execution production configuration "
            "source against its outer archive SHA-256 and six independently "
            "supplied champion pins."
        )
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        result = verify_windows_execution_production_config_source(
            args.archive,
        ...
        return 2
    print("WINDOWS_EXECUTION_PRODUCTION_CONFIG_SOURCE_VERIFIED")
