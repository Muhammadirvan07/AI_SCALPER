#!/usr/bin/env python3
"""Generate one deny-only Windows decision provider overlay."""
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Generate a deterministic four-file Windows decision provider "
            "overlay. No provider is imported or materialized."
        )
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        result = prepare_windows_decision_provider_pack(
        ...
        return 2
    print("WINDOWS_DECISION_PROVIDER_PACK_PREPARED")
    ...
    print(
        "Decision base release identity SHA-256: "
        f"{result.decision_base_release_identity_sha256}"
    )
