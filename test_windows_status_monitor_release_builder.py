...
import build_windows_status_monitor_release as status_monitor_release_builder
...
class WindowsStatusMonitorReleaseBuilderTests(unittest.TestCase):
    ...
    def test_sidecar_failure_preserves_replaced_archive_path(self):
        with tempfile.TemporaryDirectory() as raw:
            base = Path(raw)
            root, allowlist = self._repo(base)
            output = base / "status-monitor.zip"
            replacement = base / "replacement.zip"
            original_write = status_monitor_release_builder._write_exclusive
            ...
            try:
                with patch.object(
                    status_monitor_release_builder,
                    "_write_exclusive",
                    ...
                    with self.assertRaisesRegex(
                        ...
                        build_status_monitor_release(root, allowlist, output)
