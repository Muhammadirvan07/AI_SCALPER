#!/usr/bin/env python3
"""Assemble one deny-only, suite-bound configured Status Monitor candidate."""
...
def _bootstrap_release_root() -> Path:
    ...
    try:
        ...
        raise RuntimeError(
            "STATUS_MONITOR_CONFIGURED_CANDIDATE_BOOTSTRAP_REJECTED"
        ) from exc
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Assemble a suite-bound configured Status Monitor candidate "
            "without provider, credential, task, process, MT5, broker, "
            "network, SQLite, or order effects."
        )
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        result = assemble_windows_status_monitor_configured_candidate(
            ...
            status_monitor_base_release=args.status_monitor_base_release,
        ...
        reason = getattr(
            exc,
            "reason_code",
            "STATUS_MONITOR_CONFIGURED_CANDIDATE_REJECTED",
        )
        print(
        ...
        return 2
    print("WINDOWS_STATUS_MONITOR_CONFIGURED_CANDIDATE_ASSEMBLED")
