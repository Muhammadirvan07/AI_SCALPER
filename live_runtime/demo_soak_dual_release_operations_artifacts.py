"""Immutable review artifacts for dual-release demo-soak operations.

The artifact boundary is intentionally non-operational.  It accepts exact,
non-secret metadata, reconstructs the typed dual-release plan, and emits only
validation-task review material.  It has no provider loader, credential
backend, scheduler mutation, process launcher, network, MT5, or broker surface.
"""
...
from .demo_soak_dual_release_operations import (
...
INPUT_SCHEMA_VERSION = "windows-dual-release-demo-soak-operations-input-v2"
BUNDLE_SCHEMA_VERSION = (
    "windows-dual-release-demo-soak-operations-review-bundle-v2"
)
FAILURE_DRILL_MANIFEST_SCHEMA_VERSION = (
    "windows-dual-release-failure-drill-manifest-v2"
)
...
class DualReleaseOperationsArtifactError(RuntimeError):
...
def _reject_duplicates(pairs: list[tuple[str, object]]) -> dict[str, object]:
    ...
    for key, value in pairs:
        if key in result:
            raise DualReleaseOperationsArtifactError("DUPLICATE_JSON_KEY")
        result[key] = value
    return result
...
def _read_json_object(path: str | Path) -> Mapping[str, Any]:
    ...
    try:
        ...
        if (
            ...
            raise DualReleaseOperationsArtifactError(
                "DUAL_RELEASE_OPERATIONS_INPUT_FILE_INVALID"
            )
        ...
        after = source.lstat()
    except DualReleaseOperationsArtifactError:
        raise
    except OSError as exc:
        raise DualReleaseOperationsArtifactError(
            "DUAL_RELEASE_OPERATIONS_INPUT_FILE_UNAVAILABLE"
        ) from exc
    ...
    if (
        ...
        raise DualReleaseOperationsArtifactError(
            "DUAL_RELEASE_OPERATIONS_INPUT_CHANGED_DURING_READ"
        )
    try:
        document = json.loads(
        ...
        raise DualReleaseOperationsArtifactError(
            "DUAL_RELEASE_OPERATIONS_INPUT_JSON_INVALID"
        ) from exc
    if not isinstance(document, Mapping):
        raise DualReleaseOperationsArtifactError(
            "DUAL_RELEASE_OPERATIONS_INPUT_OBJECT_REQUIRED"
        )
    return document
...
def _role(value: object, *, label: str) -> ServiceReleaseRoleBinding:
...
def _plan_from_mapping(
    ...
    root = _mapping(
        ...
        label="dual_release_operations_input",
    )
    if root.get("schema_version") != INPUT_SCHEMA_VERSION:
        raise DualReleaseOperationsArtifactError(
            "DUAL_RELEASE_OPERATIONS_INPUT_SCHEMA_MISMATCH"
        )
    ...
    return WindowsDualReleaseDemoSoakOperationsPlan(
...
def load_windows_dual_release_demo_soak_operations_plan(
    path: str | Path,
) -> WindowsDualReleaseDemoSoakOperationsPlan:
    """Read and reconstruct one exact non-secret v2 operations plan."""
    ...
    try:
        ...
        if type(exc) is DualReleaseOperationsArtifactError:
            raise
        raise DualReleaseOperationsArtifactError(
            f"DUAL_RELEASE_OPERATIONS_INPUT_REJECTED:{exc}"
        ) from exc
...
class DualReleaseFailureDrillManifest:
    plan_sha256: str
...
def _manifest(
    ...
    *,
    issued_at_utc: datetime,
) -> DualReleaseFailureDrillManifest:
    return DualReleaseFailureDrillManifest(
        plan_sha256=plan.plan_sha256,
        decision_release_identity_sha256=(
            plan.decision.release_identity_sha256
        ),
        execution_release_identity_sha256=(
            plan.execution.release_identity_sha256
        ),
        decision_release_manifest_sha256=(
...
def _readiness(
    ...
    return {
        ...
        "local_dual_release_plan_valid": (
            result.local_dual_release_plan_valid
        ),
...
def build_windows_dual_release_demo_soak_review_bundle(
    plan: WindowsDualReleaseDemoSoakOperationsPlan,
    *,
    issued_at_utc: datetime,
) -> dict[str, object]:
    """Build and self-verify one deterministic v2 review bundle."""
    ...
    verify_windows_dual_release_demo_soak_review_bundle(payload)
    return payload
...
def _canonical_plan_to_input(value: object) -> dict[str, object]:
    plan = _mapping(
        ...
        label="canonical_dual_release_plan",
    )
    locks = {
        "execution_enabled": False,
        "live_allowed": False,
    ...
    if any(plan.get(name) != expected for name, expected in locks.items()):
        raise DualReleaseOperationsArtifactError(
            "CANONICAL_DUAL_RELEASE_PLAN_SAFETY_LOCK_DRIFT"
        )
    return {
...
def _rebuild_manifest(value: object) -> DualReleaseFailureDrillManifest:
    raw = _mapping(
        value,
        fields=_MANIFEST_FIELDS,
        label="dual_release_failure_drill_manifest",
    )
    return DualReleaseFailureDrillManifest(
...
def verify_windows_dual_release_demo_soak_review_bundle(
    payload: Mapping[str, object],
) -> WindowsDualReleaseDemoSoakOperationsPlan:
    """Reconstruct and verify every component of a v2 review bundle."""

    root = _mapping(
        payload,
        fields=_BUNDLE_FIELDS,
        label="dual_release_operations_bundle",
    )
    if root.get("schema_version") != BUNDLE_SCHEMA_VERSION:
        raise DualReleaseOperationsArtifactError(
            "DUAL_RELEASE_OPERATIONS_BUNDLE_SCHEMA_MISMATCH"
        )
    unsigned = dict(root)
    claimed = require_hash("content_sha256", unsigned.pop("content_sha256"))
    if canonical_sha256(unsigned) != claimed:
        raise DualReleaseOperationsArtifactError(
            "DUAL_RELEASE_OPERATIONS_BUNDLE_CONTENT_HASH_MISMATCH"
        )
    ...
    try:
        ...
        raise DualReleaseOperationsArtifactError(
            "DUAL_RELEASE_OPERATIONS_BUNDLE_PLAN_INVALID"
        ) from exc
    if root["plan"] != plan.to_dict():
        raise DualReleaseOperationsArtifactError(
            "DUAL_RELEASE_OPERATIONS_BUNDLE_PLAN_NOT_CANONICAL"
        )
    if root["plan_sha256"] != plan.plan_sha256:
        raise DualReleaseOperationsArtifactError(
            "DUAL_RELEASE_OPERATIONS_BUNDLE_PLAN_HASH_MISMATCH"
        )
    ...
    if (
        ...
        raise DualReleaseOperationsArtifactError(
            "DUAL_RELEASE_FAILURE_DRILL_MANIFEST_BINDING_MISMATCH"
        )
    ...
    if reviews != _scheduler_reviews(plan):
        raise DualReleaseOperationsArtifactError(
            "DUAL_RELEASE_SCHEDULER_REVIEW_BINDING_MISMATCH"
        )
    readiness = _mapping(
