#!/usr/bin/env python3
"""Generate one deterministic, deny-only Windows LIVE provider pack."""
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Generate an exact four-file Windows LIVE Execution provider "
            "pack without importing providers, reading credentials, "
            "opening SQLite, initializing MT5, or enabling broker mutation."
        )
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        return 2
    print("WINDOWS_LIVE_EXECUTION_PROVIDER_PACK_PREPARED")
