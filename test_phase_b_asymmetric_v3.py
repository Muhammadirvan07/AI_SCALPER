...
class AsymmetricClosureV3Tests(unittest.TestCase):
    ...
    def test_materialized_release_inventory_binding_rejects_drift(self):
        module=__import__("operator_packs.finex_trusted_utc_v1.phase_b_asymmetric_v3",fromlist=["verify_materialized"])
        with self.assertRaisesRegex(ContractError,"MATERIALIZED_RELEASE_INVENTORY_MISMATCH"):
            module.verify_materialized(self.precommit,canonical(self.loader),self.public,"finex-phase-d-operator",self.ssh,"finex-cas",self.root,"0"*64)
