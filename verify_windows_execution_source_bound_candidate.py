#!/usr/bin/env python3
"""Verify one deny-only Windows Execution source-bound candidate ZIP."""
...
def _parser() -> argparse.ArgumentParser:
    parser = _StableArgumentParser(
        description=(
            "Verify one deny-only Execution source-bound candidate against "
            "nine independent external pins."
        )
    )
    ...
    parser.add_argument("--expected-bound-archive-sha256", required=True)
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        return 2
    print("WINDOWS_EXECUTION_SOURCE_BOUND_CANDIDATE_VERIFIED")
