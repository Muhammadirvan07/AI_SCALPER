...
class GeneratorIntegrationTests(unittest.TestCase):
 ...
 def test_real_all_roles_and_prepare_phase_b_inputs(self):
  ...
  for host,key,roles in (("finex",self.keys["finex"],("finex-cas","finex-fetcher")),("putra",self.keys["putra"],("putra-producer",))):
   ...
   handoff=json.loads(out.read_text());self.assertRegex(handoff["release_inventory_sha256"],r"^[0-9a-f]{64}$")
   validator=Path(PREPARE).with_name("validate_phase_d_inputs.py");kind=host+"-phase-b-v3"
   for label,mutate in (("missing",lambda value:value.pop("release_inventory_sha256")),("extra",lambda value:value.update({"unexpected":True}))):
    changed=json.loads(json.dumps(handoff));mutate(changed);target=self._write(host+"-"+label+"-handoff.json",changed);check=subprocess.run([sys.executable,str(validator),kind,str(target)],capture_output=True,text=True,check=False);self.assertEqual(2,check.returncode)
   changed=json.loads(json.dumps(handoff));role_name=roles[0].split("-",1)[1] if host=="finex" else "producer";changed[role_name]["decoded_bindings"]["trust_binding"]["release_inventory_sha256"]="0"*64;target=self._write(host+"-inventory-drift-handoff.json",changed);check=subprocess.run([sys.executable,str(validator),kind,str(target)],capture_output=True,text=True,check=False);self.assertEqual(2,check.returncode)
   finalize_arguments=[]
   for operator_role in roles:
    short={"finex-cas":"cas","finex-fetcher":"fetcher","putra-producer":"producer"}[operator_role];materialized=self._write(host+"-"+short+"-materialized.json",{"decoded_bindings":handoff[short]["decoded_bindings"],"encoded_command":handoff[short]["encoded"],"encoded_command_sha256":handoff[short]["encoded_sha256"],"schema_version":"finex-phase-b-materialized-loader-v3"})
    argument=self._write(host+"-"+short+"-finalize.json",{"PhaseBMaterializedLoaderJson":str(materialized.resolve()),"PhaseBPublicKeyPath":str(Path(str(key)+".pub").resolve()),"PhaseBPublicKeySha256":hf(str(key)+".pub"),"PhaseBSignerIdentity":host+"-phase-d-operator","PhaseBSshKeygenPath":str(self.ssh.resolve()),"PhaseBSshKeygenSha256":hf(self.ssh),"PrecommitRoot":str(roots[operator_role].resolve()),"PythonPath":str(Path(sys.executable).resolve()),"PythonSha256":hf(sys.executable),"V3CorePath":str((self.published/"phase_b_asymmetric_v3.py").resolve()),"V3CoreSha256":hf(self.published/"phase_b_asymmetric_v3.py")});finalize_arguments.append(argument)
   build=Path(gen.__file__).with_name("BUILD_FINEX_PHASE_D.ps1");q=lambda value:"'"+str(value).replace("'","''")+"'";expected="@('finex-cas','finex-fetcher')" if host=="finex" else "@('putra-producer')";arguments="@("+",".join(q(path) for path in finalize_arguments)+")";finalize_command="& "+q(build)+" -RepoRoot "+q(Path(__file__).resolve().parent)+" -OutputRoot "+q(self.root)+" -PublishedReleaseRoot "+q(self.published)+" -FinalizePublished -UnsignedContentManifestPath "+q(self.inventory)+" -ExpectedFinalizeRoles "+expected+" -FinalizeArgumentsJson "+arguments
   finalized=subprocess.run([str(self.power),"-NoProfile","-NonInteractive","-ExecutionPolicy","Bypass","-Command",finalize_command],capture_output=True,text=True,timeout=30,check=False);self.assertEqual(0,finalized.returncode,finalized.stdout+finalized.stderr);self.assertIn("FINALIZED_WITHOUT_RELEASE_MUTATION",finalized.stdout)
