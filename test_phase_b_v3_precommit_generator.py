...
class GeneratorIntegrationTests(unittest.TestCase):
 def setUp(self):
  ...
  self.config=self._write("bindings.json",{"schema_version":"test-bindings-v1"});self.firewall=self._write("firewall.json",{"state":"absent"});self.entry=self._file("entry.py",b"entry")
  self.responder=self._file("live_runtime/windows_trusted_utc_continuity_cas_responder.py",b"responder");self.acceptance=self._file("live_runtime/windows_trusted_utc_continuity_acceptance.py",b"acceptance");self.operator=self._file("operator.py",b"operator");self.policy=self._file("acl.json",b"policy");self.acceptance_verifier=self._file("acceptance.py",b"verifier")
 def tearDown(self):
 ...
 def request(self,role):
  ...
  value={"action_execute_path":str(self.power),"attestation_path":str(self.state/(role+"-attestation.json")),"attestation_signature_path":str(self.state/(role+"-attestation.json.sig")),"config_and_key_bindings_path":str(self.config),"finex_authority_public_key_path":str(self.keys["finex"])+".pub","finex_host_identity_path":str(self.hosts["finex"][0]),"finex_host_identity_signature_path":str(self.hosts["finex"][1]),"finex_joint_binding_signature_path":str(self.joint_sigs["finex"]),"firewall_path":str(self.firewall),"future_pointer_path":str(self.state/(role+"-current.json")),"generation_id":str(list(gen.ROLE).index(role)+1)*32,"joint_binding_path":str(self.binding),"observer_path":str(PACK/"PHASE_B_V3_WINDOWS.ps1"),"operator_role":role,"precommit_root":str(self.state/(role+"-precommit")),"predecessor_generation_id":"0"*32,"private_key_path":str(authority),"putra_authority_public_key_path":str(self.keys["putra"])+".pub","putra_host_identity_path":str(self.hosts["putra"][0]),"putra_host_identity_signature_path":str(self.hosts["putra"][1]),"putra_joint_binding_signature_path":str(self.joint_sigs["putra"]),"python_path":str(Path(sys.executable).resolve()),"readiness_challenge_path":str(self.state/(role+"-challenge.json")),"readiness_public_key_path":str(ready)+".pub","readiness_receipt_path":str(self.state/(role+"-readiness.json")),"release_identity_path":str(self.release_path),"release_root":str(PACK),"runtime_arguments":{"named":self._base_named(role,ready),"positionals":[]},"runtime_path":str(PACK/gen.ROLE[role]["runtime"]),"runtime_state_root":str(self.state),"schema_version":"finex-phase-b-v3-precommit-generator-request-v1","sequence":1,"ssh_keygen_path":str(self.ssh),"task_user_id":"HOST\\operator","v3_core_path":str(PACK/"phase_b_asymmetric_v3.py")}
