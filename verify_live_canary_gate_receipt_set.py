"""Independently verify a persisted LIVE-canary gate receipt set."""
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Verify one deny-only nine-domain LIVE-canary gate set"
    )
    parser.add_argument("--binding", type=Path, required=True)
    parser.add_argument("--trust-policy", type=Path, required=True)
    parser.add_argument("--receipt-set", type=Path, required=True)
    parser.add_argument(
        "--evidence",
        action="append",
        default=[],
    ...
    try:
        ...
        receipts = verify_live_canary_gate_receipt_set(
            _rooted(args.receipt_set),
            binding,
            policy,
