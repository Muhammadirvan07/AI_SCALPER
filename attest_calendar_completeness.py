"""Attest the reviewed final source inventory for one calendar chain."""
...
from validation_evidence import EvidenceValidationError, attest_calendar_completeness
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Attest a final reviewed calendar-source inventory"
    )
    ...
    try:
        ...
        request = load_completeness_request(
        ...
        attestation = attest_calendar_completeness(
            ...
            profile.contract_id,
            attestation_id=str(request["attestation_id"]),
            attested_at=trusted_now,
            expected_final_head_hmac_sha256=str(
                request["expected_final_head_hmac_sha256"]
            ),
        ...
        print("CALENDAR_COMPLETENESS_REJECTED: " + code)
        print("Safety lock remains active; no broker order was submitted.")
        return 2
    print("Calendar completeness attested: " + str(attestation["attestation_id"]))
    print("Candidate: " + profile.candidate_id)
    print("Contract: " + profile.contract_id)
    print(
        "Final amendment HMAC: "
        + str(attestation["final_amendment_head_hmac_sha256"])
    )
