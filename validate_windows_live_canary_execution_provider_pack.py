#!/usr/bin/env python3
"""Validate a Windows LIVE provider pack without importing providers."""
...
def _bootstrap_release_root() -> Path:
    ...
    try:
        ...
        raise RuntimeError(
            "LIVE_EXECUTION_PROVIDER_TOOLING_BOOTSTRAP_REJECTED"
        ) from exc
...
from live_runtime.windows_execution_provider_pack_generator import (  # noqa: E402
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Validate an exact Windows LIVE Execution provider pack without "
            "provider import, credential access, SQLite, MT5, process, "
            "network, task, broker, or order effects."
        )
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        result = validate_windows_live_canary_execution_provider_pack(
        ...
        reason = getattr(
            exc,
            "reason_code",
            "LIVE_EXECUTION_PROVIDER_PACK_REJECTED",
        )
        print(
            f"WINDOWS_LIVE_EXECUTION_PROVIDER_PACK_REJECTED: {reason}",
            file=sys.stderr,
        )
        return 2
    print("WINDOWS_LIVE_EXECUTION_PROVIDER_PACK_VALID")
