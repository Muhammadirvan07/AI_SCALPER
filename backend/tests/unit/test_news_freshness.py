...
@pytest.mark.parametrize(
    ("hours", "status"),
    [
        (1, NewsFreshnessStatus.REALTIME),
        (71, NewsFreshnessStatus.REALTIME),
        (72, NewsFreshnessStatus.REALTIME),
        (73, NewsFreshnessStatus.RECENT),
        (167, NewsFreshnessStatus.RECENT),
        (169, NewsFreshnessStatus.HISTORICAL),
    ],
)
def test_freshness_boundaries(hours: int, status: NewsFreshnessStatus) -> None:
...
def test_missing_timezone_and_clock_skew_are_handled_safely() -> None:
    ...
    assert aware.age_hours == 4 and aware.status == NewsFreshnessStatus.REALTIME
