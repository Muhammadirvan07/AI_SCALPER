#!/usr/bin/env python3
"""Prepare one deny-only Windows LIVE source-bound candidate ZIP."""
...
def _parser() -> argparse.ArgumentParser:
    parser = _StableArgumentParser(
        description=(
            "Prepare one deterministic deny-only Windows LIVE candidate "
            "bound to exact reviewed DEMO source ancestry."
        )
    )
    ...
    parser.add_argument(
        "--demo-source-bound-archive", type=Path, required=True
    )
    parser.add_argument(
        "--live-configured-candidate-root", type=Path, required=True
    )
    parser.add_argument(
...
def _print_result(result: object) -> None:
    print("WINDOWS_LIVE_CANARY_EXECUTION_SOURCE_BOUND_CANDIDATE_READY")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        result = prepare_windows_live_canary_execution_source_bound_candidate(
            base_suite_root=args.base_suite_root,
            execution_base_release=args.execution_base_release,
            demo_source_bound_archive=args.demo_source_bound_archive,
            live_configured_candidate_root=(
                args.live_configured_candidate_root
            ),
