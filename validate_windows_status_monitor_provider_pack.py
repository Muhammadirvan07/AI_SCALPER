#!/usr/bin/env python3
"""Validate one Status Monitor provider pack without importing its factory."""
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Validate a generated Windows Status Monitor provider overlay "
            "without provider, credential, SQLite, request, process, MT5, "
            "broker, or order effects."
        )
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        return 2
    print("WINDOWS_STATUS_MONITOR_PROVIDER_PACK_VALID")
