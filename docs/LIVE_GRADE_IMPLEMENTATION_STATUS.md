...
| 3. Broker read-only shadow | FBS selected and exact binding configured; preflight pending; evidence not started | Probe sanitasi mengikat exact server `FBS-Demo`, company `FBS Markets Inc.`, akun demo USD 500:1 retail hedging, serta XAUUSD/EURUSD/USDJPY/AUDUSD tanpa suffix. Default preflight, diagnostic, dan report diarahkan ke FBS. FINEX tidak dipakai untuk observasi baru. Instrument specification API, calendar, 20-session benchmark, regulatory eligibility, dan promotion evidence tetap pending. |
