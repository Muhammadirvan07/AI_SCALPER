...
Validasi lokal terakhir pada 2026-07-29 menjalankan **1.841 test** tanpa
...
| 6. XAUUSD live canary | Activation, prebootstrap, portable custody, dan sealed runtime launch-session boundaries implemented locally; canary not started | Exact broker/demo/live/release/champion binding, policy-pinned promotion/gate/human/deployment authorities, three distinct approvals, five-minute trusted-clock window, atomic one-use SQLite registry, exact DDL/trigger verification, dan signed off-host rollback checkpoint tersedia secara source. Prebootstrap v1 menambah complete non-secret LIVE runtime candidate, exact sealed DEMO Execution source-bound ancestry, runtime/activation key separation, trusted-time recheck, dan sealed status `PREBOOTSTRAP_EVIDENCE_COMPLETE_CENTRAL_UNLOCK_REQUIRED`. Portable custody v1 menambah public RSA WORM receipt verification dengan byte-exact admission readback, separately signed checkpoint/CAS acknowledgement, independently retained predecessor pin, lane/key separation, durable nonce readback, rollback rejection, serta sealed one-use launch prerequisite. Runtime launch-session v1 kemudian memerlukan seluruh exact prerequisite dan pin, dua observasi current external state, atomic process-local replay consumption, serta central LIVE policy yang benar pada awal/akhir. Checked-in central policy tetap false sehingga session tidak dapat diterbitkan. Jika policy kelak dibuka setelah seluruh bukti diterima, session hanya memberi launch/bootstrap authority; execution dan broker mutation tetap false serta per-order guards tetap wajib. Existing Windows source-bound v1 tetap DEMO dan tidak direlabel LIVE. Actual XM demo-auto cohort 30 hari/50 fill/20 XAU, exact XM/Windows LIVE candidate, LIVE promotion receipt, sembilan gate eksternal, tiga approval manusia aktual, real independent Windows/WORM readback dan CAS receipt, production bootstrap/supervisor integration, central unlock ceremony, serta first live canary belum ada. Lima puluh closed live trades adalah bukti pascacanary untuk ekspansi, bukan syarat membuat order canary pertama. |
