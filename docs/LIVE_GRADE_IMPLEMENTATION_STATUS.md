...
| 6. XAUUSD live canary | Deny-only evidence boundary implemented locally; canary not started | Exact broker/demo/live/release/champion binding, policy-pinned promotion/gate/human/deployment authorities, three distinct approvals, five-minute trusted-clock window, atomic one-use SQLite registry, exact DDL/trigger verification, dan signed off-host rollback checkpoint tersedia secara source. Seluruh artifact tetap `live_allowed=false`, `activation_authorized=false`, dan `order_capability=DISABLED`. Actual XM demo-auto cohort 30 hari/50 fill/20 XAU, LIVE promotion receipt, sembilan gate eksternal, tiga approval manusia aktual, Windows/WORM checkpoint custody, runtime integration, central unlock, serta first live canary belum ada. Lima puluh closed live trades adalah bukti pascacanary untuk ekspansi, bukan syarat membuat order canary pertama. |
