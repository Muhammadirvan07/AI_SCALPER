...
| 3. Broker read-only shadow | FBS transition requested; exact binding probe required; evidence not started | Operator meminta target dipindah dari FINEX ke FBS. Probe sanitasi read-only tersedia untuk mengambil exact demo server, currency, leverage, margin mode, dan alias XAUUSD/EURUSD/USDJPY/AUDUSD tanpa credential atau account identifier. FINEX tidak dipakai untuk observasi baru. FBS belum boleh menjalankan diagnostic sampai hasil probe direview, config binding dipatch, dan preflight FBS lulus. Seluruh output tetap non-promotional dan bukan validation evidence. |
