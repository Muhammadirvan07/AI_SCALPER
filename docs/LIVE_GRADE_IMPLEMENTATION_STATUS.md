...
| 6. XAUUSD live canary | Deny-only activation and prebootstrap evidence boundaries implemented locally; canary not started | Exact broker/demo/live/release/champion binding, policy-pinned promotion/gate/human/deployment authorities, three distinct approvals, five-minute trusted-clock window, atomic one-use SQLite registry, exact DDL/trigger verification, dan signed off-host rollback checkpoint tersedia secara source. Prebootstrap v1 menambah complete non-secret LIVE runtime candidate, exact sealed DEMO Execution source-bound ancestry, runtime/activation key separation, trusted-time recheck, dan sealed status `PREBOOTSTRAP_EVIDENCE_COMPLETE_CENTRAL_UNLOCK_REQUIRED`. Seluruh artifact tetap `bootstrap_authorized=false`, `live_allowed=false`, `activation_authorized=false`, dan `order_capability=DISABLED`; existing Windows source-bound v1 tetap DEMO dan tidak direlabel LIVE. Actual XM demo-auto cohort 30 hari/50 fill/20 XAU, exact XM/Windows LIVE candidate, LIVE promotion receipt, sembilan gate eksternal, tiga approval manusia aktual, portable Windows/WORM admission custody, one-use launch capability, effect-capable runtime integration, central unlock, serta first live canary belum ada. Lima puluh closed live trades adalah bukti pascacanary untuk ekspansi, bukan syarat membuat order canary pertama. |
