...
| 3. Broker read-only shadow | Phillip Commodity Window 02 contract registered; transfers V1--V5 verified, V6 install remediation pending | Phillip Commodity contracts v1--v5 and the historical V6 scheduler remain immutable failed history; the old V6 task stays `Disabled`. Windows registered exact Window 02 contract `phillip-commodity-window-02-diagnostic-v1`, snapshot `phillip-commodity-dev-pre-window-02-v1`, payload `cbfd753b0aed2d66af56446adc734ce8d62666e309e91bf74d24b4cc56b613a2`, contract hash `ad4fd8853563976483fbffbd3bd97847f7e05c8a4194afd10fa95832e2fe485b`, build identity `9d64b8c9be0b42bdc991b767a745258774a57f80613e2fd322791d6d18cc6287`, and signing-key ID `105e393cd619804e`. V1 stopped on PowerShell native-stderr promotion; V2 reached the authoritative verifier but exposed a projection mismatch; V3 corrected the projection and exposed the persistent lock; V4 bound the operational inventory but was not executed; V5 added the correct two-phase eight-to-nine-file proof, then target Windows exposed a function-scope shadow caused by assigning `$null` to automatic `$LASTEXITCODE` before the first Git call. `WINDOW02.V6` removes all automatic-variable assignments, keeps immediate exit-code/output capture, and uses fresh create-exclusive `-r6` paths while preserving V1--V5. Limited-token V6 installation, first automatic run, full window, completeness attestation, seal, and post-window acceptance remain pending. No manual start or broker mutation is allowed; order capability remains `DISABLED`; Phillip FX remains registration-disabled. |
