...
| 3. Broker read-only shadow | Phillip Commodity V5/V6 failed closed; Window 02 rollover review tooling implemented locally | FBS forex/metal/crypto diagnostic domains dan Phillip FX/commodity dual-terminal lanes memiliki journal/report terpisah. Phillip Commodity sanitized discovery-v3, signed regulatory/calendar reviews, dan registration gate telah disetujui. Contract v1 tetap immutable/kosong setelah legacy expert-flag HOLD. Contract v2 membuktikan HMAC source chain tetapi startup melebihi append grace; v3 mempertahankan repeated-activation failure. V4 membuktikan 13 authenticated children tetapi task gagal tertutup pada optional `RunLevel`. V5 membuktikan 12 authenticated children dan one dependency session; task V5 kemudian disabled ketika Windows menghilangkan XSD-default `StartWhenAvailable=false` dan validator StrictMode lama membacanya secara dinamis. V6 mempertahankan frozen worker/contract/journal/proof V5 dan membuat task/evidence scheduler baru dengan shared XSD+CIM validator. Automatic boundary 4 Agustus terbukti dipicu Task Scheduler tetapi selesai dengan result `2`. Verifikasi Administrator terhadap kontrak V5 valid, sedangkan verifikasi identik dari token non-Administrator mereproduksi `SNAPSHOT:SNAPSHOT_INVALID:PermissionError`; ini membuktikan ACL snapshot tidak mencakup principal task. ACL saja tidak dapat menyelamatkan V5 karena XAUUSD masih memiliki nol segmen dan first append deadline 26 Juli telah lewat. Remediasi lokal kini menyimpan exact verifier failures di authenticated operational metadata, menyediakan quiesce script yang menonaktifkan task lama dan memberi SID exact read/execute sambil membuktikan snapshot bytes tetap sama, serta menambahkan review-only Window 02 untuk 17 Agustus--13 Oktober JST. Corrected R2 calendar dan regulatory reviews sudah tersedia; additive tooling kini memverifikasi semua HMAC/lane binding dan menghasilkan exact candidate/profile/signed-template after-images sebagai paket eksternal create-exclusive tanpa apply, contract registration, scheduler mutation, atau broker effect. V5/V6 tetap failed immutable evidence dan tidak boleh diklaim acceptance. Paket R2 harus dibuat dan direview di Windows sebelum after-images, contract Window 02, snapshot, dan scheduler baru boleh diproses. Order capability tetap `DISABLED`; Phillip FX tetap registration-disabled. |
