...
LOCAL_REGRESSION = 1485_OF_1485_PASS_NORMAL_AND_OPTIMIZED
...
- Provider foundation dan generator tidak ditambahkan ke execution,
  read-only-shadow, atau status-monitor release.
...
base-release foundation, generated provider bytes, contract hash, role, dan
canonical non-secret configuration. Hash tersebut tidak diterima dari input
operator.
...
1. siapkan generic configured-overlay descriptor;
2. bangun suite-bound configured decision release;
3. lakukan provider-conformance dan independent validation;
4. terbitkan external RSA launcher attestation;
5. review Task Scheduler identity/ACL dan Credential Manager ACL;
6. kumpulkan sembilan signed pre-manual observations;
7. baru masuk 10 controlled manual-demo lifecycle.
