#!/usr/bin/env python3
"""Validate one deny-only Windows LIVE Execution candidate."""
...
def _bootstrap_release_root() -> Path:
    ...
    try:
        ...
        raise RuntimeError(
            "LIVE_EXECUTION_CONFIGURED_CANDIDATE_BOOTSTRAP_REJECTED"
        ) from exc

    def reparse(item: object) -> bool:
        return bool(int(getattr(item, "st_file_attributes", 0)) & 0x400)
...
from live_runtime.windows_live_canary_execution_configured_candidate import (  # noqa: E402
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Validate one immutable Windows LIVE Execution configured "
            "candidate without provider import, credential access, SQLite, "
            "MT5, network, task installation, process start, or broker "
            "mutation."
        )
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        result = validate_windows_live_canary_execution_configured_candidate(
        ...
        reason = getattr(
            exc,
            "reason_code",
            "LIVE_EXECUTION_CONFIGURED_CANDIDATE_REJECTED",
        )
        print(
            "WINDOWS_LIVE_EXECUTION_CONFIGURED_CANDIDATE_REJECTED: "
            f"{reason}",
            file=sys.stderr,
        )
        return 2
    print("WINDOWS_LIVE_EXECUTION_CONFIGURED_CANDIDATE_VALID")
