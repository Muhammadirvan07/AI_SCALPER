...
After both disabled/no-trigger installs produce their real v3 topology evidence, rerun the same command with a new `-PreparationRoot C:\Users\muham\AI_SCALPER_RELEASES\phase-d-finalized` and append `-PostInstallInputsJson <post-install-v3.json>`. The finalized pack emits attest, exact-pointer publish, and activation commands; no separate Phase C input is accepted. Existing preparation roots are immutable and never resumed or overwritten.
