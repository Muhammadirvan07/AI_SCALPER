"""Generate a verified read-only performance report for crypto shadowing."""
...
DEFAULT_OUTPUT = (
    REPO_ROOT / "runtime_state" / "diagnostic" / "crypto-weekend-performance.json"
)


def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Build a read-only, non-promotional BTC/ETH shadow report"
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    report = build_crypto_diagnostic_report(args.database)
    _write_report(report, args.output)
    overall = report["overall"]
    if not isinstance(overall, Mapping):
        raise DiagnosticReportError("generated report metrics are unavailable")
    print(f"Crypto diagnostic report written: {args.output}")
...
def cli_entrypoint(argv: Sequence[str] | None = None) -> int:
    try:
        ...
        print(f"CRYPTO_DIAGNOSTIC_REPORT_REJECTED: {exc}", file=sys.stderr)
        return 2
