...
class PhaseDPackTests(unittest.TestCase):
    def test_fail_closed_and_no_private_export_or_activation(self):
        ...
        self.assertIn("Get-ScheduledTask",status);self.assertIn("Get-NetFirewallRule",status)
        self.assertIn("ContentManifestSha256",status);self.assertIn("PhaseBEncodedCommand",status)
        self.assertIn("FromBase64String",build+putra)
        self.assertIn("SetAccessRuleProtection($true,$false)",build+putra)
        self.assertIn("S-1-5-18",build+putra);self.assertIn("content.directories",status)
        self.assertIn("expected_task_logon_type='Interactive'",build)
        self.assertIn("install-receipt.json",status)
    ...
    def test_preparers_are_status_first_key_safe_and_do_not_install(self):
        ...
        self.assertIn("$publishedConfig=Join-Path $dest",finex)
    ...
    def test_external_handoff_validator_is_closed_and_canonical(self):
        ...
        encoded="YWJj";digest=hashlib.sha256(encoded.encode()).hexdigest()
        value={"schema_version":"putra-phase-d-phase-b-input-v1","encoded":encoded,
               "encoded_sha256":digest,"pointer":r"C:\\protected\\current.json",
               "pointer_sha256":"1"*64}
        with tempfile.TemporaryDirectory() as directory:
            path=Path(directory)/"input.json"
            path.write_bytes((json.dumps(value,sort_keys=True,separators=(",",":"))+"\n").encode())
            self.assertEqual(value,module.load("putra-phase-b",path))
            for raw in ('{"encoded":"a","encoded":"b"}\n',json.dumps(value)+"\n",
                        json.dumps({**value,"extra":1},sort_keys=True,separators=(",",":"))+"\n"):
                path.write_bytes(raw.encode())
                with self.assertRaises((ValueError,json.JSONDecodeError)):
                    module.load("putra-phase-b",path)
    ...
    def test_all_external_contract_modes_have_exact_schema_dispatch(self):
        ...
        module=importlib.util.module_from_spec(spec);spec.loader.exec_module(module)
        self.assertEqual({"finex-phase-b","finex-phase-b-v2","finex-post-install","finex-phase-c",
                          "putra-phase-b","putra-phase-b-v2","putra-post-install"},set(module.SCHEMAS))
        for kind,(schema,fields) in module.SCHEMAS.items():
            self.assertIn("schema_version",fields)
            self.assertRegex(schema,r"-v[12]$")
            self.assertEqual(len(fields),len(set(fields)))

    def test_phase_b_v2_requires_semantic_plan_cross_links(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(handoff,module.load("putra-phase-b-v2",path))
            handoff["producer"]["signer_identity"]="finex-phase-d-operator";path.write_bytes(module.canonical(handoff))
            with self.assertRaises(ValueError):module.load("putra-phase-b-v2",path)
            value["receipt_signer_identity"]="finex-phase-d-operator";plan.write_bytes(module.canonical(value));handoff["producer"]["plan_manifest_sha256"]=hashlib.sha256(plan.read_bytes()).hexdigest();path.write_bytes(module.canonical(handoff))
            with self.assertRaises(ValueError):module.load("putra-phase-b-v2",path)
    ...
    def test_precommit_sources_are_deterministic_and_private_key_free_publisher(self):
        ...
        for marker in ("New-PhaseBPrecommitPlan","GenerationId","PredecessorGenerationId","FUTURE_POINTER_HASH_REQUIRED","finex-phase-b-precommit-plan-v1"):self.assertIn(marker,phase)
        for marker in ("PRECOMMIT_SIGNATURE_INVALID","PHASE_C_ANTI_ROLLBACK_CONFLICT","[IO.File]::Replace","PHASE_C_LIVE_TOPOLOGY_DRIFT","PHASE_C_REPLACED_PREDECESSOR_DRIFT","OpenOrCreate","ShouldProcess"):self.assertIn(marker,publisher)
        self.assertNotIn("ReceiptPrivateKeyPath",publisher);self.assertNotIn("[Guid]::NewGuid",publisher)
        for marker in ("ed25519_fingerprint","key_evidence_signature","joint_binding","generation_receipt_sha256"):self.assertIn(marker,planner)
        self.assertIn("expected_signer",(PACK/"validate_phase_d_inputs.py").read_text("utf-8"))
        self.assertIn("$pointer.sequence-lt1",phase)
    ...
    def test_host_identity_binding_and_sequence_contracts(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertIn("PHASE_B_PRECOMMIT_PUBLISHER_CONTRACT_REQUIRED",status["blockers"])
    ...
    def test_preparer_status_subprocess_is_canonical_and_nonmutating(self):
        ...
        for name,schema in (("PREPARE_FINEX_PHASE_D_LOCAL.ps1","finex-phase-d-preparation-status-v1"),
            ...
            with self.subTest(script=name),tempfile.TemporaryDirectory() as profile:
                ...
                before=list(Path(profile).iterdir())
                ...
                self.assertEqual(before,list(Path(profile).iterdir()))
